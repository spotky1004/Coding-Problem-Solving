...
const deltaTimeLoseL = new LazySegmentTree({
  defaultValue: 0,
  values: Array(M).fill(0),
  mergeFunc: (l, r) => l + r,
  queryFuncs: [
    (el, idx, s, e) => [el + 2 * (e - s + 1), 2]
  ],
  lazyMergeFunc: (curLazy, lazy) => curLazy + lazy,
  lazyApplyFunc: (el, lazy, idx, s, e) => el + lazy * (e - s + 1)
});
const deltaTimeLoseR = new LazySegmentTree({
  defaultValue: 0,
  values: Array(M).fill(0),
  mergeFunc: (l, r) => l + r,
  queryFuncs: [
...
for (const cat of cats) {
  const [s, e] = [cat[1], cat[cat[0]]].sort((a, b) => a - b);
  if (s !== 1) deltaTimeLoseL.updateRange(0, 1, s - 1);
  if (e !== M) deltaTimeLoseR.updateRange(0, e + 1, M);
}

const timeLoses = Array(M + 1).fill(0);
timeLoses[0] = Infinity;
let lTimeLose = 0;
for (let i = M; i >= 1; i--) {
  lTimeLose += deltaTimeLoseL.sum(i, i);
  timeLoses[i] += lTimeLose;
}
let rTimeLose = 0;
for (let i = 1; i <= M; i++) {
  rTimeLose += deltaTimeLoseR.sum(i, i);
  timeLoses[i] += rTimeLose;
}

let minPos = 0;
for (let i = 1; i <= M; i++) {
  if (timeLoses[minPos] > timeLoses[i]) minPos = i; 
}
