...
countSegTree.update(0, 1);
let nextNum = 2;
for (const [pi, xi] of queries) {
  const startPos = numPos[pi];
  let l = -1, r = N;
  const fixedXi = ((xi - 1) % countSegTree.sum(1, N)) + 1;
  while (l + 1 < r) {
    const m = Math.floor((l + r) / 2);
    let s = startPos + 1, e = s + m;
    if (e > N) e -= N;
    let emptyCount = 0;
    if (s <= e) emptyCount = countSegTree.sum(s, e);
    else emptyCount = countSegTree.sum(s, N) + countSegTree.sum(1, e);
    if (emptyCount < fixedXi) l = m;
    else r = m;
  }
  const nextPos = (startPos + r + N) % N;
  seq[nextPos] = nextNum;
  numPos[nextNum] = nextPos;
  countSegTree.update(0, nextPos + 1);
  nextNum++;
}
