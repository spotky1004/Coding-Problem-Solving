...
const SNums = Array.from(S).map(v => parseInt(v, 36) - 10);
// const leftSegTree = new LazySegmentTree({
//   values: SNums,
//   defaultValue: -1,
//   lazyApplyFunc: (el, lazy) => (el + lazy) % 26,
//   lazyMergeFunc: (cur, acc) => cur + acc,
//   mergeFunc: l => l,
//   queryFuncs: [
//     (el) => {
//       el++;
...
//   ]
// });
// const rightSegTree = new LazySegmentTree({
//   values: SNums,
//   defaultValue: -1,
//   lazyApplyFunc: (el, lazy) => (el + lazy) % 26,
//   lazyMergeFunc: (cur, acc) => cur + acc,
...
/**
 * [groupCount: number, leftValue: number, rightValue: number]
 */
const groupCountSegTree = new LazySegmentTree({
  values: SNums.map(v => [1, v, v]),
  defaultValue: 0,
  mergeFunc: (l, r) => {
    if (l === 0) return r;
    if (r === 0) return l;
    let newGroupCount = l[0] + r[0];
    if (l[2] === r[1]) newGroupCount--;
    return [newGroupCount, l[1], r[2]];
  },
  lazyApplyFunc: (el, lazy) => [el[0], (el[1] + lazy) % 26, (el[2] + lazy) % 26],
  lazyMergeFunc: (cur, acc) => cur + acc,
  queryFuncs: [
    el => [[el[0], (el[1] + 1) % 26, (el[2] + 1) % 26], 1]
  ]
});
...
for (const [type, l, r] of queries) {
  if (type === 1) {
    out.push(groupCountSegTree.sum(l, r)[0]);
  } else {
    groupCountSegTree.updateRange(0, l, r);
  }
}
