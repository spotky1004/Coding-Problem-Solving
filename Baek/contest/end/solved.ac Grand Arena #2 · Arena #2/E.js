...
for (let i = 1; i < datas.length; i += 2) {
  for (const b of datas[i]) {
    uniques.add(b);
  }
}
const sorted = [...uniques].sort((a, b) => b - a);
const sortedIdx = new Map(sorted.map((v, i) => [v, i + 1]));
...
for (let i = 0; i < datas.length; i += 2) {
  ...
  const bUniques = new Set();
  for (const b of B) {
    bUniques.add(b);
  }
  const bSorted = [...bUniques].sort((a, b) => b - a);
  const bSortedIdx = new Map(bSorted.map((v, i) => [v, i + 1]));
  
  const bCountSegTree = new SegmentTree({
    values: Array(N).fill(0n),
    defaultValue: 0n,
    mergeFunc: (left, right) => left + right,
    queryFuncs: [
      (el) => (el + 1n) % p
  ...
  for (const b of B) {
    const idx = sortedIdx.get(b);
    const bIdx = bSortedIdx.get(b);

    count = (
      count +
      (idx === 1 ? 0n : countSegTree.sum(1, idx - 1) * K) +
      (bIdx === 1 ? 0n : bCountSegTree.sum(1, bIdx - 1) * K)
    ) % p;

    bCountSegTree.update(0, bIdx);
  }
  for (const b of B) {
    const idx = sortedIdx.get(b);
    const bIdx = bSortedIdx.get(b);
    ...
    count = (
      count +
      (bIdx === 1 ? 0n : bCountSegTree.sum(1, bIdx - 1) * (K - 0n) * (K - 1n) / 2n)
    ) % p;
    
    countSegTree.update(0, idx, K);
  }
}
