...
const n = datas.splice(0, Number(N)).map(v => v[0]);
const queries = datas;

const segTree = new LazySegmentTree(
  n,
  0n,
  (left, right) => (left ?? 0n) + (right ?? 0n),
  (cur, curLazy, updateValue, start, end) => ((curLazy ?? 0n) + (updateValue ?? 0n)) * BigInt(end - start + 1)
);

const out = [];
for (const [type, p1, p2, p3] of queries) {
  if (type === 1n) {
    segTree.updateRange(Number(p1), Number(p2), p3);
  } else if (type === 2n) {
    out.push(segTree.sum(Number(p1), Number(p2)) + "");
  }
}
