...
S 1 3`
  ...
  .trim()
  .split("\n")
  .map(line => line.replaceAll("M", 1).replaceAll("P", 2).replaceAll("S", 3).split(" ").map(Number));
...
const segTree = new LazySegmentTree({
  /** @type {[sum: number, min: number]} */
  values: H.map(v => [v, v]),
  defaultValue: null,
  mergeFunc: (left, right) => {
    if (right === null) return [...left];
    if (left === null) return [...right];
    return [left[0] + right[0], Math.min(left[1], right[1])];
  },
  queryFuncs: [
    (el, idx, s, e, v) => [[el[0] + v * (e - s + 1), el[1] + v], v]
  ],
  lazyMergeFunc: (curLazy, lazy) => (curLazy ?? 0) + lazy,
  lazyApplyFunc: (el, lazy, idx, s, e) => [el[0] + lazy * (e - s + 1), el[1] + lazy]
});

const out = [];
for (const [type, a, b, c] of queries) {
  if (type === 1) {
    out.push(segTree.sum(a, b)[1]);
  } else if (type === 2) {
    segTree.updateRange(0, a, b, c);
  } else if (type === 3) {
