...
// input
const [[N], can5, can2, can1, [M], queries] = input
  .trim()
  .split("\n")
...
const scores = [null];
const canIdxes = Array(N).fill(0);
/** @type {Heap<[col: number, canAcc: number]>} */
const colHeap = new Heap((a, b) => a[1] - b[1]);
for (let col = 0; col < N; col++) colHeap.push([col, can5[col]]);
let scoreAcc = N;
for (let floor = 0; floor < 6e6; floor++) {
  while (colHeap.size > 0 && colHeap.first()[1] <= floor) {
    const [col, canAcc] = colHeap.pop();
    const canIdx = canIdxes[col];

    let newCanAcc = canAcc;
    if (canIdx === 0) {
      newCanAcc += can2[col];
      scoreAcc += 1;
    } else if (canIdx === 1) {
    ...
    canIdxes[col]++;
    colHeap.push([col, newCanAcc]);
  }
  scores.push(scoreAcc);
}

const countSegTree = new SegmentTree({
  values: Array(3e6).fill(1),
  defaultValue: 0,
  mergeFunc: (l, r) => l + r,
  queryFuncs: [
    () => 0
...
const out = [];
for (const floor of queries) {
  let l = 0, r = 3e6 + 1;
  while (l + 1 < r) {
    const m = Math.floor((l + r) / 2);
    if (countSegTree.sum(1, m) < floor) l = m;
    else r = m;
  }
  const originalFloor = r;
  countSegTree.update(0, originalFloor);
  out.push(scores[originalFloor]);
}
