...
// input
const [[N, Q], ...datas] = input
  .trim()
  .split("\n")
  .map(line => line.split(" ").map(Number));
const parents = datas.splice(0, N - 1).flat();
parents.unshift(-1, -1);
const queries = datas;
...
childs.map(c => c.sort((a, b) => a - b));

let remapIdxAcc = 1;
const remapNodeIdxes = Array(N + 1).fill(-1);
const nodeRanges = Array.from({ length: N + 1 }, (_, i) => [i, i]);
function dfs(cur = 1) {
  const remapIdx = remapIdxAcc++;
  remapNodeIdxes[cur] = remapIdx;
  const start = remapIdx;
  let end = remapIdx;
  for (const child of childs[cur]) {
    end = dfs(child);
  }
  nodeRanges[remapIdx] = [start, end];
  return end;
}
dfs();

const rootSegTree = new LazySegmentTree({
  values: Array(N).fill(1),
  defaultValue: 1,
  queryFuncs: [
    (el, idx, s, e, v) => [Math.max(el, v), v]
  ],
  mergeFunc: (l, r) => Math.max(l, r),
  lazyApplyFunc: (el, lazy) => Math.max(el, lazy),
  lazyMergeFunc: (curLazy, lazy) => Math.max(curLazy, lazy),
});

const out = [];
const isRemoved = Array(N + 1).fill(false);
isRemoved[1] = true;
for (const [b, c, d] of queries) {
  const bRemapIdx = remapNodeIdxes[b];
  const cRemapIdx = remapNodeIdxes[c];
  const bRoot = rootSegTree.sum(bRemapIdx, bRemapIdx);
  const cRoot = rootSegTree.sum(cRemapIdx, cRemapIdx);
  const isConnected = bRoot === cRoot;
  out.push(isConnected ? "YES" : "NO");
  if (d === 1) {
    const toRemove = isConnected ? bRemapIdx : cRemapIdx;
    if (isRemoved[toRemove]) continue;
    const [s, e] = nodeRanges[toRemove];
    rootSegTree.updateRange(0, s, e, toRemove);
    isRemoved[toRemove] = true;
  }
}
