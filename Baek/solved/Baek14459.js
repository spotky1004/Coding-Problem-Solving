...
const l = mokchos.splice(0, N);
const r = mokchos;
const rSorted = r
  .map((v, i) => [v, i])
  .sort((a, b) => a[0] - b[0]);

const dpSegTree = new SegmentTree({
  defaultValue: 0,
...
for (let i = 0; i < N; i++) {
  const lType = l[i];
  const from = Math.max(1, lType - 4);
  const to = Math.min(N, lType + 4);
  const rIdxes = [];
  for (let rType = from; rType <= to; rType++) {
    rIdxes.push(rSorted[rType - 1][1]);
  }
  rIdxes.sort((a, b) => b - a);
  for (let i = 0; i < rIdxes.length; i++) {
    const rIdx = rIdxes[i];
    if (rIdx === 0) {
      dpSegTree.update(0, rIdx + 1, 1);
      continue;
    }
    const curVal = dpSegTree.sum(rIdx + 1, rIdx + 1);
    const newVal = dpSegTree.sum(1, rIdx) + 1;
    if (curVal === newVal) continue;
    dpSegTree.update(0, rIdx + 1, newVal);
  }
  // console.log(Array.from({ length: N }, (_, i) => dpSegTree.sum(i + 1, i + 1)));
}

// output
return dpSegTree.sum(1, N);
}
