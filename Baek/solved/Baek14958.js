...
me.reverse();
const rMul = convolution(opp.map(v => Number(v === "S")), me.map(v => Number(v === "R")));
const pMul = convolution(opp.map(v => Number(v === "R")), me.map(v => Number(v === "P")));
const sMul = convolution(opp.map(v => Number(v === "P")), me.map(v => Number(v === "S")));

let max = 0;
for (let i = me.length - 1; i < rMul.length; i++) {
  max = Math.max(
    max,
    rMul[i] + pMul[i] + sMul[i]
  );
}
