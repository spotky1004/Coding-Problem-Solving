...
const { BLOCK, FLOOR } = GridWorker.CELL_PROPS;

const gw = new GridWorker({
  dim: 2,
  grid: board,
  moves: [
    [1, 0], [-1, 0],
...
const gramPos = GridWorker.finder(2, board, 2).get(2)[0];
const dists = gw.calcDistances([0, 0], [N - 1, M - 1]);

let minDist = dists[N - 1][M - 1];
if (minDist === -1) minDist = Infinity;

const gramDist = dists[gramPos[0]][gramPos[1]];
if (gramDist !== -1) minDist = Math.min(minDist, gramDist + Math.abs((N - 1) - gramPos[0]) + Math.abs((M - 1) - gramPos[1]));

// output
return minDist === -1 || minDist > T ? "Fail" : minDist;
}
