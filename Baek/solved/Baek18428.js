...
const teacherPos = GridWorker.finder(2, board, "T").get("T");

function check() {
  for (const [i, j] of teacherPos) {
    for (let ip = i + 1; ip < N; ip++) {
      const cell = board[ip][j];
      if (cell === "O" || cell === "T") break;
      if (cell === "S") return false;
    }
    for (let ip = i - 1; ip >= 0; ip--) {
      const cell = board[ip][j];
      if (cell === "O" || cell === "T") break;
      if (cell === "S") return false;
    }
    for (let jp = j + 1; jp < N; jp++) {
      const cell = board[i][jp];
      if (cell === "O" || cell === "T") break;
      if (cell === "S") return false;
    }
    for (let jp = j - 1; jp >= 0; jp--) {
      const cell = board[i][jp];
      if (cell === "O" || cell === "T") break;
      if (cell === "S") return false;
    }
...
const n2 = N * N;
const idxToPos = (idx) => [idx % N, Math.floor(idx / N)];
for (let a = 0; a < n2; a++) {
  const [i, j] = idxToPos(a);
  if (board[i][j] !== "X") continue;
  board[i][j] = "O";
  for (let b = a + 1; b < n2; b++) {
    const [i, j] = idxToPos(b);
    if (board[i][j] !== "X") continue;
    board[i][j] = "O";
    for (let c = b + 1; c < n2; c++) {
      const [i, j] = idxToPos(c);
      if (board[i][j] !== "X") continue;
      board[i][j] = "O";
      if (check()) return "YES";
      board[i][j] = "X";
