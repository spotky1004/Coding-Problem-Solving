...
// cases
check(`2463534242 723471715`,
`2`);
check(`2463534242 1232801914`,
`4294967294`);
check(`2463534242 2463534242`,
`1`);
}
...
for (let i = 1; i <= SIZE / 2; i++) {
  const prev = aPows[i - 1];
  aPows.push(matMul(prev, prev));
}
...
let rVec = numToVec(t);
for (let i = 0; i < B; i++) {
  r.set(vecToNum(rVec), i);
  rVec = applyMat(rVec, A);
}

let shiftCount = -1;
lVec = numToVec(x);
const bLog2 = Math.log2(B);
for (let i = 0; i < B; i++) {
  const lNum = vecToNum(lVec);
  if (r.has(lNum)) shiftCount = (i * B - r.get(lNum) + MAX_VAL) % MAX_VAL;
  lVec = applyMat(lVec, aPows[bLog2]);
}
