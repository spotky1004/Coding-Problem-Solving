...
const nFactors = primeFactorization(n, primes);
const nDivisors = getDivisors(nFactors);

let ans = -1;
for (const [x, xFactorsData] of nDivisors) {
  const xFactors = xFactorsData.map(v => v[0]);
