...
const out = Array.from({ length: N }, _ => Array(N).fill(0));

const heap = new Heap((a, b) => b[1] - a[1]);
for (let i = 0; i < N; i++) if (degs[i] > 0) heap.push([i, degs[i]]);

while (heap.size > 0) {
  const [u, uDeg] = heap.pop();

  const tmp = [];
  if (heap.size < uDeg) return -1;
  for (let i = 0; i < uDeg; i++) {
    const [v, vDeg] = heap.pop();
    out[u][v] = 1;
    out[v][u] = 1;
    if (vDeg !== 1) tmp.push([v, vDeg - 1]);
  }
  while (tmp.length > 0) heap.push(tmp.pop());
}
