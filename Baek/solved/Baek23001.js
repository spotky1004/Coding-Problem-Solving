...
const out = [];
let line = 0;
for (let tcNr = 0; tcNr < T; tcNr++) {
  const [R, C] = lines[line++];
  const board = lines.slice(line, line + R);
  line += R;

  const heap = new Heap((a, b) => b[2] - a[2]);
  for (let r = 0; r < R; r++) {
    for (let c = 0; c < C; c++) {
      heap.push([r, c, board[r][c]]);
    }
  }
  let ans = 0;
  while (heap.size > 0) {
    const [r, c, curCount] = heap.pop();
    if (board[r][c] !== curCount) continue;
    for (const [dr, dc] of directions) {
      ...
      const nextCount = board[tr][tc];
      const diff = curCount - nextCount;
      if (diff <= 1) continue;
      ans += diff - 1;
      board[tr][tc] = curCount - 1;
      heap.push([tr, tc, curCount - 1]);
    }
  }
  out.push(`Case #${tcNr + 1}: ${ans}`);
}
