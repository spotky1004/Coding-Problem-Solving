...
const maxCompValue = comp.size;
const compA = A.map(ai => comp.get(ai));

const countSegTree1 = new SegmentTree({
  values: Array(maxCompValue).fill(0),
  defaultValue: 0,
  mergeFunc: (l, r) => l + r,
  queryFuncs: [
    (el, v) => el + v
...
const rightBiggerCounts = Array(N).fill(0);
for (let i = 0; i < S; i++) {
  countSegTree1.update(0, compA[i], 1);
}
for (let i = S; i < N; i++) {
  if (compA[i - S] !== maxCompValue) {
    rightBiggerCounts[i - S] = countSegTree1.sum(compA[i - S] + 1, maxCompValue);
  }
  countSegTree1.update(0, compA[i - S], -1);
  countSegTree1.update(0, compA[i], 1);
}

const countSegTree2 = new SegmentTree({
  values: Array(maxCompValue).fill(0),
  defaultValue: 0,
  mergeFunc: (l, r) => l + r,
  queryFuncs: [
...
for (let i = 0; i < N; i++) {
  if (i >= S) {
    countSegTree2.update(0, compA[i - S], -1);
    curCount -= rightBiggerCounts[i - S];
  }
  countSegTree2.update(0, compA[i], 1);
  if (compA[i] !== 1) curCount += countSegTree2.sum(1, compA[i] - 1);
