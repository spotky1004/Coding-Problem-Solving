...
check(`408`, `120888`);
check(`1022`, `520212`);
check(`23421`, `274117920`);
check(`499999`, `125974521990`);
check(`500000`, ``);
}
...
for (let i = 1; i < n; i++) f[i * i % n]++;
const mul = convolution(f, f, p, r);
const left = Array(n).fill(0);
for (let i = 1; i < n; i++) left[2 * i * i % n]++;
for (let i = 0; i < mul.length; i++) left[i % n] += mul[i];
for (let i = 0; i < n; i++) left[i] /= 2;
let count = 0;
for (let i = 1; i < n; i++) count += left[i * i % n];
