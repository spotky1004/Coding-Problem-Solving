...
const grid = new GridWorker({
  dim: 2,
  grid: board,
  cellProps: new Map([
    [".", GridWorker.CELL_PROPS.FLOOR],
    ["*", GridWorker.CELL_PROPS.BLOCK]
  ]),
  moves: [
    [-1, -1], [-1, 0], [-1, 1],
    [0, -1], [0, 1],
...
let idx = 0;
const flood = grid.flood((value) => value === "*" ? "*" : idx++);
const counts = Array(board.length * board[0].length).fill(0);
for (const cell of flood.flat()) if (cell !== "*") counts[cell]++;
