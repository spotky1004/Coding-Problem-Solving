...
const m = 100000000000000000000000000000000000n;
const [, ...questions] = (
  !isDev
...
`
  ...
  .trim()
  .split("\n")
  .map(line => line.split(" ").map(t => BigInt(t)));

function sqrt(a) {
  const _m = m**2n;
  const fixedNum = a * _m**2n;
  if (fixedNum < 0) return 0n;
  const nIterCount = 4000;
...
for (const [a, b, c, d] of questions) {
  ...
  for (let i = 1; i < ranges.length; i++) {
    ...
    out.push(Number(ans*100000000n/m)/1e8);
  }
  console.log([...new Set(out.sort((a, b) => a - b).map(v => v.toFixed(5)))].join(" "));
}
