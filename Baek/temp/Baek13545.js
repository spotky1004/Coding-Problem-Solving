...
const prefixSum = [0];
for (let i = 0; i < N; i++) prefixSum.push(prefixSum[i] + A[i]);

const OFFSET = 100000;
const idxDatas = Array.from({ length: 200001 }, () => [[0, 0], []]);
for (let i = 0; i < prefixSum.length; i++) idxDatas[prefixSum[i] + 100000][1].push(i);
...
for (let [queryIdx, [i, j]] of sortedQueries) {
  ...
  while (l > i) {
    const val = prefixSum[l] + OFFSET;
    const [range, idxes] = idxDatas[val];
    if (idxes[range[0] - 1] >= l && range[0] !== 0) {
      const prevLen = idxes[range[1]] - idxes[range[0]];
      lenCounts[prevLen]--;
      if (lenCounts[prevLen] === 0) maxSegTree.update(0, prevLen, 0);
