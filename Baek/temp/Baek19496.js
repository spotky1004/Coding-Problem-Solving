...
async function interactiveJudger(data) {
  ...
  while (true) {
    const [type, received] = interactiveReceiver().split(" ");
    if (type === "?") await interactiveSender((received % m).toString());
    else if (type === "!") {
      if (Number(received) !== m) return { type: "WA" };
      return { type: "AC" };
    }
  }
}
...
while (l + 1 < r) {
  ...
  if (
    (await interactive(`? ${m}`)) === m &&
    (await interactive(`? ${m - 1}`)) === m - 1
  ) l = m;
  else r = m;
}
await interactive(`! ${l + 1}`);
