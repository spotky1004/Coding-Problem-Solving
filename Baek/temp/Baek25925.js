...
// input
const [[N, Q], ...points] = input
  .trim()
  .split("\n")
  .map(line => line.split(" ").map(Number));
const queries = points.splice(N, Q);
...
const lValueIdxes = new Map(
  [...new Set(lValues)]
    .sort((a, b) => a - b)
    .map((v, i) => [v, i + 1])
);
const rValueIdxes = new Map(
  [...new Set(rValues)]
    .sort((a, b) => a - b)
    .map((v, i) => [v, i + 1])
);

const lMaxSegTree = new SegmentTree({
  values: Array(lValueIdxes.size).fill(0),
  defaultValue: 0,
...
const rMaxSegTree = new SegmentTree({
  values: Array(rValueIdxes.size).fill(0),
  defaultValue: 0,
  mergeFunc: (l, r) => Math.max(l, r),
  queryFuncs: [
    (e, v) => Math.max(e, v)
...
dp[0] = 1;
lMaxSegTree.update(0, lValueIdxes.get(points[0][0] - points[0][1]), 1);
rMaxSegTree.update(0, rValueIdxes.get(points[0][0] + points[0][1]), 1);
for (let i = 1; i < N; i++) {
  const [Xi, Yi] = points[i];
  const lIdx = lValueIdxes.get(Xi - Yi);
  const rIdx = rValueIdxes.get(Xi + Yi);
  const maxValue = Math.max(
    lMaxSegTree.sum(1, lIdx),
    rMaxSegTree.sum(rIdx, rValueIdxes.size)
  ) + 1;
  dp[i] = maxValue;
  lMaxSegTree.update(0, lIdx, maxValue);
  rMaxSegTree.update(0, rIdx, maxValue);
}

const out = [];
for (const [qi] of queries) {
  out.push(dp[pointIdxes[qi - 1]]);
}
